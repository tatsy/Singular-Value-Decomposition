...
public:
    PTHREAD_QUEUE(const int thread_count)
        : number_of_threads(thread_count), threads(new pthread_t[thread_count]), active_threads(thread_count),
          inactive_threads(0), queue(65535) {
    ...
    static void *Thread_Routine(void *data) {
        ...
        while (1) {
            ...
            while (queue.queue.Empty()) {
                ...
                if (queue.active_threads == 0)
                    pthread_cond_signal(&queue.done_condition);
